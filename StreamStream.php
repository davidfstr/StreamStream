...
class StreamStream
{
     ...
     *                              For example "stream://5016d2d5c9d90".
     */
    public static function createUriForStream($stream)
    {
        $streamId = uniqid();
        StreamStream::$registeredStreams[$streamId] = $stream;
    ...
    {
        return count(StreamStream::$registeredStreams);
    }
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (array_key_exists($streamId, StreamStream::$registeredStreams))
        {
            $this->stream = StreamStream::$registeredStreams[$streamId];
    ...
    public function stream_close()
    {
        ...
        if (isset(StreamStream::$registeredStreams))
        {
            unset(StreamStream::$registeredStreams[$this->streamId]);
        }
    }
    ...
    public function url_stat($path, $flags)
    {
        ...
        if (array_key_exists($streamId, StreamStream::$registeredStreams))
        {
            $stream = StreamStream::$registeredStreams[$streamId];
...
stream_wrapper_register('stream', 'StreamStream') or 
